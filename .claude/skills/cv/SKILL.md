...
3. **一條 bullet 一個成就**：禁止把多句話塞進同一條 `highlights`/`contributions` 陣列字串裡（例如「做了 A。也做了 B。還做了 C。」）。每個成就獨立成一條，1-2 行為佳；塞太多句子會讓「量化比例」這類統計指標失真，也違反 ATS 逐條解析的預期。**例外**：`Project.contributions` 改以非條列式單一段落撰寫，不適用本點與第 1 點的 XYZ 公式，詳見第 3 節「專案介紹 (Projects)」的「`Project.contributions`（重大貢獻）撰寫規範」。
...
8. **每份經歷/專案的 bullet 上限為 3 條**：`Experience.highlights`、`Project.highlights` 各自陣列**最多 3 個項目**（原本項目數 ≤3 則不受影響）；`Project.contributions` 現為非條列式單一段落字串（僅 1 條，天然 ≤ 3，寫法見第 3 節「專案介紹 (Projects)」的「`Project.contributions`（重大貢獻）撰寫規範」）。若 `/source` 或既有素材中該經歷/專案的候選 bullet（`highlights`）超過 3 條，只留下以下優先順序中最強的 3 條，其餘捨棄，不得為了保留資訊而硬塞成一條或超過 3 條：
...
- **一律排除「亞洲遊」這段經歷**：`/source` 中的「亞洲遊 - 日本市場業務及營運」（2016.09–2017.09，日本旅宿平台業務／營運，非工程職）不寫入任何生成的履歷，也不列為「僅標題」段落。這是軟體工程職涯之前的非工程性質工作，列入對軟體工程職缺的 ATS 沒有加分，反而稀釋工程時間軸。此排除為預設規則，不需要再向使用者確認。造成的時間軸空檔不需額外說明。

### 3. 專案介紹 (Projects)

#### 數量
- **固定選 4 個**與該 JD 最相關的專案，說明技術亮點與達成成果，不多不少。
- **4 個名額全部留給與 JD Required 直接對應的專案**；與 JD 無直接對應的專案一律不列入，不為填滿名額而降低平均相關性。若對應 Required 的強專案不足 4 個，寧可少寫也不硬湊。
...
#### 產品力／熱情名額（選用）
- 4 個名額中可保留**最多 1 個**給「有真實使用者數據的個人專案」，作為主動打造好用產品的佐證（例如上架商店的擴充套件、有 DAU/WAU 與評價數據的個人作品）。
- 該專案仍須至少對應一條 JD 職責或 Preferred，不能純粹為了展示熱情而佔用名額。
- 放在最後一個位置，不佔用前 3 個對應 Required 的名額；若 4 個對應 Required 的專案都很強，直接放棄此名額。

#### 內容規範
- 每個專案的 `highlights` **最多 3 條**，見文字撰寫規範第 8 點的篩選順序；`contributions` 改為非條列式單一段落字串（僅 1 條，天然 ≤ 3），寫法見下方「`Project.contributions`（重大貢獻）撰寫規範」。
- **`Project.contributions`（重大貢獻）撰寫規範**：
  - 以**非條列式的單一段落**呈現：`contributions` 陣列只放 **1 個字串**，內容是一段完整散文（不切成多條、不加句尾句號）。
  - **重點數字加粗**：段落中的關鍵數字（量化成果、規模、成長率、使用者數據，如 `**90%**`、`**1,000**`、`**50%**`）用 `**...**` 標記；其餘文字一律不加粗。渲染端（`projects-grid.tsx` 與純表格 `plain-resume.tsx`）都會把 `**...**` 轉成 `<strong>`。
  - 段落內容只陳述：**結果** ＋ **關鍵前提條件** ＋ **關鍵技術手段**。不描述原理、實作步驟或過程細節。
  - 重點取捨權重（由高到低，字數有限時優先保留高權重要素）：
    1. **獨立開發**（一人／完全獨立／從零打造）
    2. **帶領**（領導團隊、跨團隊主導）
    3. **數字相關**（量化成果、規模、成長率、使用者數據）
    4. **開源**（GitHub 開源、上架商店、公開專案）
  - 此規範是「一條 bullet 一個成就」與「每條 bullet 套 XYZ 公式」規則的**例外**，僅適用於 `contributions`。
  - **`Project.highlights`（技術亮點）不受此規範影響**：維持最多 3 條的條列式寫法。
...
`skillKeywordRows` 需**針對 JD 篩選與排序**，不再整塊複製基礎範本。原則如下：

1. **JD 匹配優先排序**：JD 直接匹配的關鍵字（Required／Preferred 命中、JD 職責明列的技術、以及 JD 反覆強調的主題）放在**最前面的幾個 row**；其後才排前後端主流技能。
2. **只留主流技能**：其餘名額只保留前後端業界廣泛使用、面試官一眼能辨識的技能——主流框架與語言、主流狀態管理／資料請求／表單／驗證、主流測試／建置／打包工具、主流雲端／DB／CI。
3. **刪除冷門技能**：`/source` 或基礎範本中出現、但屬**小眾、已淘汰、或只在單一舊專案用過**的關鍵字一律不寫（例如冷門 UI 框架、冷門測試 runner、單一專案才用到的函式庫、少見的 CI／模板引擎／RPC／協定、PWA 邊角 API 等）。
4. **不得新增**：只能從 `/source`／基礎範本既有的關鍵字中挑選，不可自行補上任何範本沒有的技能。
5. **JD 明列技能不可漏**：篩完後逐一走過 JD 的技能清單（Required + Preferred + 職責段落點名的工具），確認**每一項都有進 `skillKeywordRows`**——這是本區塊唯一的硬性下限。
6. 每個 JD 匹配的關鍵字，應同時出現在至少一條經歷或專案 bullet 中（呼應第五階段的 `keyword_distribution`）。

> **為什麼改**：舊版要求「與基礎範本逐字等量複製、不做篩選」，是為了防止漏掉 JD 明列的 Preferred 技能；但這會讓每份履歷都塞滿 130+ 個含大量冷門項目的關鍵字，稀釋 JD 匹配訊號、也讓 ATS 關鍵字密度失真。新版改為「JD 匹配優先 + 只留主流、砍掉冷門」，並以第 5 點的人工核對確保 JD 明列技能不遺漏。若基礎範本本身落後於 `/source`（`/source` 有新主流技能但範本沒有），仍應先跑 `/update` 同步範本，再篩選。
...
可執行 `pnpm check-cv {uuid}` 自動檢查第 2–7 項與第 14 項（見 `scripts/check-cv.ts`）；第 1 項因改為 JD 篩選，僅自動確認 `skillKeywordRows` 區塊存在且非空，篩選是否合理需人工核對。該指令不可用時，其餘項目逐項人工核對。

| # | 檢查項目 | 判定標準 |
|---|---|---|
| 1 | 技能關鍵字篩選 | `skillKeywordRows` 已依「JD 匹配優先 → 前後端主流技能」排序，**不含冷門／已淘汰／單一舊專案專用**關鍵字；且 JD 明列的每項技能都有收錄（見第二階段 §5） |
| 2 | bullet 數量上限 | 所有 `highlights` 陣列**各自 ≤ 3 條**；`contributions` 為非條列式單一段落字串（僅 1 條，天然 ≤ 3） |
...
| 14 | 工作經歷完整性 | `/source` 的工作經歷**全部**列出（「亞洲遊 - 日本市場業務及營運」為預設排除、經同意排除者除外），最近 4 段有 `highlights`、第 5 段以後為 `highlights: []` |
