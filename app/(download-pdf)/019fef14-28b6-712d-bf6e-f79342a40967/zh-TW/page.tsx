...
export const metadata: Metadata = {
  title: "John Hsieh | 應徵 Full Stack Software Engineer I @ Booking.com",
  description:
    "7 年前端與全端工程經驗，具備 Node.js/React/Vue 全端技術棧與獨立端到端產品交付經驗——應徵 Booking.com for Business Full Stack Software Engineer I 職缺。",
...
const contacts: ContactItem[] = [
  ...
  { href: "https://lazy-cv.com/019fef14-28b6-712d-bf6e-f79342a40967/zh-TW", type: "website", label: "個人網站" },
];
...
const experiences: Experience[] = [
  {
    ...
    highlights: [
      "以 React 17、TypeScript 於 User Growth 團隊主導廣告投放財務結算平台，架構橫跨 50+ 子專案、百萬行程式碼規模的 Rush monorepo，影響大量廣告主使用者",
      "整理廣告主與內部使用者每日透過 oncall 反饋的問題與需求，主動發起對應的技術型開發任務，而非只被動等待需求指派",
      "將 CSV 校驗邏輯透過 Comlink RPC 轉移至 Web Worker 執行，減少 90% UI 卡頓時間，並抽象為共用內部工具",
      "打造模仿 React useMemo 的 cache/cacheMany 快取工具，降低 98% Total Blocking Time (TBT)",
  ...
  },
  {
    ...
    highlights: [
      ...
      "指導 2 位新進工程師並獲得正向回饋，同時主導內部 NextJS 教學論壇，在新人 onboarding 過程中扮演重要角色",
      "投入 JSTF 跨團隊小組，制定全公司前端開發準則，並導入 Lighthouse CI 自動化把關效能",
    ],
  },
  {
    company: "AI.Book 愛相簿",
    ...
    highlights: [
      "以獨立契約工程師身分端到端主導線上相片書編輯器，獨立工作並自行做出所有技術決策——包含涵蓋 OAuth 第三方登入、購物車訂單流程、金流串接的後端 API 設計",
      "從消費端視角獨立定義 API 契約、資料模型與錯誤處理，6 個月內完成完整產品上線",
    ],
  },
  {
    company: "FundFluent",
    ...
    highlights: [
      "從公司成立初期加入，獨立規劃前端架構與技術選型，使用 React / NextJS、TypeScript 開發面向香港市場的中小企貸款平台",
  ...
  },
  {
    ...
    highlights: [
      "於新成立的 AI 團隊中擔任全端工程師並管理 2 人前端團隊，設計並維護整合 PDF 處理程式庫的 Web 應用 API 與服務（Node.js/Express、MongoDB、Redis）",
    ],
  },
];
...
const projects: Project[] = [
  {
    ...
    summary:
      "一款讓台灣／新加坡使用者觀看網頁版 Bilibili 更順暢的 Chrome / Edge / Firefox 瀏覽器擴充套件，動態將影片 CDN host 重排為當地最快節點，具備自動 fallback 機制。從技術架構、開發到上架與維護皆為個人獨立完成，所有技術決策皆獨立做出。",
    ...
    highlights: [
      "MAIN world 注入攔截 fetch/XHR 與 playinfo 物件 setter，即時改寫 playurl/DASH manifest 中的 CDN host，且不影響頁面原生行為",
    ...
    contributions: [
      ...
      "上架 10 天內，週活躍使用者（WAU）即突破 1,000（**留存率近 100%**），獲得 **14 筆五星好評、零負評**，全數為自然流量，影響大量使用者",
  ...
  },
  {
    name: "LINE 購物",
    summary:
      "LINE 導購服務，凡經由 LINE 購物外連到合作夥伴商家完成消費後即可獲得點數回饋。全端項目，包含 Vue SSR 前端倉庫與整合數十個後端微服務的 BFF（NodeJS + GraphQL），服務大量客戶群。",
  ...
  },
  {
    name: "UG 海外增長業務平台",
    summary:
      "大型企業級 Monorepo 前端專案，服務 TikTok 線上廣告海外增長業務線，涵蓋媒體資產管理、財務結算、供應商採購、數據看板等多個業務領域，管理 50+ 子專案，百萬級程式碼規模，影響大量內部與廣告主使用者。",
  ...
  },
  {
    name: "AI.Book 相簿編輯器",
    summary:
      "線上相片書編輯器，支援照片拖曳/尺寸調整/文字編輯，跨平台支援含 PWA 安裝體驗，背後由端到端設計的金流串接結帳流程支撐，全程由唯一工程師獨立完成。從零規劃到上線僅 6 個月。",
    ...
    contributions: [
      "從零開始單獨開發，端到端擁有從結帳體驗到後端金流 API 契約的完整切片，全程獨立做出技術決策",
    ],
  },
  {
    ...
    contributions: [
      "作為金融科技放貸產品的唯一開發者，從零開始獨立完成，服務持續成長的中小企客戶群",
    ],
  },
  {
    name: "Lazy CV",
    summary:
      "AI 驅動的客製化履歷生成系統，結合 AI Agent 自動化與 Next.js 16 全端應用——只需提供職缺連結，即可自動抓取 JD 並生成量身定做的雙語履歷網頁，背後由 Neon PostgreSQL 資料庫支撐。從零到上線僅花費 2 天。",
    ...
    highlights: [
      "Neon Serverless PostgreSQL + Drizzle ORM 實現 type-safe 的關聯式資料庫存取",
...
export default function BookingComFullStackResumeZhTW() {
  return (
    <ResumeLayout>
      ...
      <HeroProfile
        ...
        avatarFallback="JH"
        jobTitle={"軟體工程師 - TikTok\n應徵 Full Stack Software Engineer I - Booking.com"}
        description={`7 年前端與全端經驗，現於 TikTok，先前 LINE Taiwan，應徵 Booking.com Full Stack Software Engineer I。核心技術棧對應職缺要求的 Node.js/React/Vue/HTML/CSS，具 PostgreSQL 關聯式資料庫與 Express 後端 API 經驗。於 AI.Book、FundFluent 兩段契約中擔任唯一工程師，獨立做技術決策交付產品。現主導 TikTok 廣告財務結算平台，於百萬行 monorepo 中解決浮點數精度問題，快取工具降低 98% TBT。

具新人指導經驗：於 LINE 60 人審查團隊中指導 2 位新人並主導教學論壇，帶領 4 人團隊完成架構遷移。善於將廣告主每日反饋歸納為開發任務。中英日三語流利。`}
        location="Singapore"
        stats={stats}
        contacts={contacts}
        globeCaption="TikTok Singapore / LINE Taiwan — 全端工程與獨立產品交付經驗"
      />
      ...
      <ProjectsGrid
        title="專案介紹"
        description={`${projects.length} 個代表性專案，展示 Node.js/React/Vue 全端交付能力、獨立產品負責經驗與規模化實績。`}
        projects={projects}
