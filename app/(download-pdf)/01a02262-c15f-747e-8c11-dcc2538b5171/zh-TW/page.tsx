...
export const metadata: Metadata = {
  ...
  description:
    "7 年 React/TypeScript 生產環境開發經驗，擅長為高流量金融平台交付高品質、可維護的使用者介面。現任 TikTok Singapore 資深軟體工程師。",
...
const contacts: ContactItem[] = [
  ...
  { href: "https://lazy-cv.com/01a02262-c15f-747e-8c11-dcc2538b5171/zh-TW", type: "website", label: "個人網站" },
];
...
const experiences: Experience[] = [
  {
    ...
    highlights: [
      "為 User Growth 團隊的廣告投放平台（高流量的財務結算與採購產品）打造 React 17 + TypeScript 前端系統，該平台位於 50+ 子專案、百萬行程式碼規模的 Rush monorepo 中",
      "透過 JSON Schema 動態表單渲染引擎交付高品質、可維護的複雜財務表單介面，涵蓋級聯選擇、動態欄位與條件驗證，並實作自訂浮點數精度方案確保報價準確性",
      "將 CSV 校驗透過 Comlink RPC 轉移至 Web Worker 執行，搭配 Yup 與 Regex 驗證規則，在 10 筆資料填寫流程中減少 90% UI 卡頓時間",
      "打造模仿 React useMemo 的 cache/cacheMany 快取工具，在一次渲染 500 個表單的介面上降低 98% Total Blocking Time (TBT)",
      "帶領開發團隊導入 Scrum，透過 retrospective 建立自我糾錯機制、以 refinement 平衡工作量，提升 PM 對開發時程的掌握度",
    ],
  },
  {
    company: "LINE Taiwan",
    ...
    highlights: [
      "開發並維護 LINE 購物（LINE 電商導購平台），技術棧為 Vue SSR、GraphQL、TypeScript 與 ExpressJS——獨立開發可覆用的「直播購物」影片自動預覽模組，透過全局調度器解決高速捲動時的預覽混亂與多直播源並發造成的記憶體暴增問題",
      "帶領 4 人團隊主導 CMS 從 PHP 重構至 React + NextJS App Router 架構，交付一套可擴充、可維護的替代系統，達成前後端完全分離",
      "投入 JSTF 跨團隊小組，制定全公司前端工具與程式碼規範（Webpack、靜態分析、PR review 要求），並導入 Lighthouse CI 自動化效能評量",
  ...
  },
  {
    ...
    highlights: [
      "獨自開發響應式線上相片書編輯器，支援桌面與行動裝置編輯，涵蓋拖曳、尺寸調整、文字編輯等功能",
      "從零規劃技術架構，使用 Vue + NuxtJS，6 個月內完成產品上線",
      "主導後端 API 設計，涵蓋 OAuth 第三方登入、購物車與訂單流程、金流串接，從消費端（前端）視角定義 API 契約與資料模型",
      "支援 PWA 安裝，於網頁與行動裝置端提供類原生 APP、可離線使用的體驗",
    ],
  },
  {
    company: "FundFluent",
    ...
    highlights: [
      "從零規劃前端架構與技術選型，於公司初創期加入，使用 React (Next.js) + TypeScript",
      "交付面向香港市場的響應式中小企貸款平台，SME Portal 與 Partner Portal 共用可擴充、可維護的 UI",
      "規劃跨專案共用 UI 組件庫（Stencil.js Web Component），不受框架限制，部署到 private NPM",
    ],
  },
  {
    ...
    highlights: [
      "管理 2 人前端團隊，於新成立的 AI 團隊中擔任全端工程師，從零規劃 SaaS 自動化平台架構",
      "以 Next.js + TypeScript + ElementUI 打造可維護的前端，整合 PDF 轉檔、翻譯、編輯與簽名等微服務於統一介面",
    ],
  },
];
  ...
  return (
    <ResumeLayout>
      ...
      <HeroProfile
        ...
        avatarFallback="JH"
        jobTitle={"資深軟體工程師 - TikTok\n應徵 Front-end Developer (Futures) - Binance"}
        description={"擁有 7 年 React/TypeScript 前端開發經驗，其中 AI.Book 與 FundFluent 兩段遠端契約案皆需高度自主、快節奏地在創新環境中獨立推進。AI.Book 由零開始獨立設計開發並上線相簿編輯器，涵蓋前端架構、編輯器核心與後端 API 設計，6 個月內上線。FundFluent 於初創期加入，獨立規劃前端架構與技術選型，交付貸款平台與跨專案共用 UI 組件庫。\n\n另具備 TikTok 資深工程師經驗，於大型 monorepo 中以 React 打造財務結算系統，熟悉 Redux、MobX 等狀態管理與 React Query 非同步處理。於 LINE Taiwan 打造響應式應用，並帶領 4 人團隊將 PHP CMS 重構為 React/Next.js。"}
        location="Singapore"
        stats={stats}
        contacts={contacts}
