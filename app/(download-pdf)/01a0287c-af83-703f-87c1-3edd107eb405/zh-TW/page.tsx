...
export const metadata: Metadata = {
  ...
  description:
    "7 年 TypeScript 開發經驗，橫跨 React/Next.js 與 Node.js，深度使用 GitHub Copilot Agent Mode，具備 CMS 重構為 Next.js 的實戰經驗。現任 TikTok Singapore 資深軟體工程師。",
...
const contacts: ContactItem[] = [
  ...
  { href: "https://lazy-cv.com/01a0287c-af83-703f-87c1-3edd107eb405/zh-TW", type: "website", label: "個人網站" },
];
...
const experiences: Experience[] = [
  {
    ...
    highlights: [
      "整理歸納廣告主與內部使用者每天透過 oncall 反饋的問題與需求共性，主動發起對應的技術型開發任務，而不只是被動等待需求指派",
  ...
  },
  {
    ...
    highlights: [
      ...
      "帶領 4 人團隊主導 CMS 從 PHP 重構至 NextJS App Router 架構，達成內容平台與舊有後端的前後端完全分離",
      "投入 JSTF 跨團隊小組，制定全公司前端開發準則，並將 Lighthouse CI 導入 CI pipeline，在每個 PR 中自動化評量效能",
    ],
  },
  {
    company: "AI.Book 愛相簿",
    ...
    highlights: [
      "獨自從零規劃技術架構並開發線上相片書編輯器，使用 Vue + NuxtJS，6 個月內完成產品上線",
      "主導後端 API 設計，涵蓋 OAuth 第三方登入、購物車與訂單流程、金流串接，從消費端（前端）視角定義 API 契約與資料模型",
      "端到端獨自負責整條技術產品線——前端架構、編輯器核心到後端 API 設計，皆由一人主導",
    ],
  },
  {
    company: "FundFluent",
    ...
    highlights: [
      ...
      "使用 React / NextJS、TypeScript、TailwindCSS 開發面向香港市場的中小企貸款平台（UI Kit、SME Portal、Partner Portal）",
      "規劃跨專案共用 UI 組件庫（Stencil.js Web Component），部署到 private NPM 供各專案重複使用",
    ],
  },
  {
    ...
    highlights: [
      ...
      "開發一個類似 n8n 的 SaaS 自動化平台，整合 PDF 轉檔、翻譯、編輯、簽名等服務與多種觸發器，前端使用 Next.js/TypeScript，後端使用 Node.js/TypeScript/MongoDB/Redis/NestJS/GraphQL",
    ],
  },
];
...
const projects: Project[] = [
  {
    ...
    highlights: [
      ...
      "AI Agent 自動化 SOP：透過 Chrome DevTools MCP 自動爬取 JD，結合結構化 Prompt 實現端到端履歷生成流程——與日常開發使用的 GitHub Copilot Agent Mode 是同一套工作模式",
  ...
  return (
    <ResumeLayout>
      ...
      <HeroProfile
        ...
        avatarFallback="JH"
        jobTitle={"資深軟體工程師 - TikTok\n應徵 Software Engineer II - Organic Growth - Microsoft"}
        description={"擁有 7 年 TypeScript 開發經驗，現任 TikTok Singapore 資深軟體工程師。深度使用 GitHub Copilot 打造 AI Agent 流程（Lazy CV、n8n）。\n\n於 LINE Taiwan 主導 LINE 購物 CMS 由 PHP 重構為 TypeScript Next.js 架構，帶 4 人團隊完成前後端分離。也曾獨立為 AI.Book、FundFluent 交付完整產品。\n\n效能實績：快取工具降 98% TBT，Web Worker 搭配 Jest 測試分流減 90% UI 卡頓；於 LINE Taiwan 導入 Lighthouse CI，並於 TikTok 參與 oncall，主導 retrospective 建立自我糾錯機制。\n\n後端同樣以 TypeScript 為主，涵蓋 Node.js、GraphQL、OAuth、Docker；樂於學習新 API，已整合 Claude、OpenAI、Gemini。"}
        location="Singapore"
        stats={stats}
        contacts={contacts}
