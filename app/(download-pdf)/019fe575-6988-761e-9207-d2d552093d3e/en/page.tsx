...
export const metadata: Metadata = {
  title: "John Hsieh | SDK Engineer - JavaScript @ Supabase",
  description:
    "7 years of TypeScript/JavaScript engineering experience with a track record of authoring shared component libraries, type-safe API layers, and publicly shipped developer tools — applying for SDK Engineer - JavaScript at Supabase.",
...
const contacts: ContactItem[] = [
  ...
  { href: "https://lazy-cv.com/019fe575-6988-761e-9207-d2d552093d3e/en", type: "website", label: "Personal Website" },
];
...
const experiences: Experience[] = [
  {
    ...
    highlights: [
      "Owned the ads-settlement frontend for the User Growth team, working across a Rush monorepo spanning 50+ sub-projects and a million-line codebase (React 17, TypeScript)",
      "Abstracted a Web Worker + Comlink validation pipeline (Yup/Regex schemas) into shared internal tooling beyond its original CSV use case, cutting UI blocking time 90% across large data-entry workflows",
      "Designed cache/cacheMany memoization utilities mimicking React's useMemo and published them as reusable internal tooling, reducing Total Blocking Time 98% on a 500-form interface",
      "Led the team's move to Scrum, using retrospectives and refinement sessions to give PMs and engineers async, written visibility into delivery timelines across a distributed team",
    ],
  },
  {
    company: "LINE Taiwan",
    ...
    highlights: [
      "Independently built a reusable video auto-preview module with a global scheduler for LINE Shopping's Live Shopping feature (Vue SSR, GraphQL, TypeScript, ExpressJS), eliminating chaotic previews and memory spikes from concurrent streams",
      "Authored the technical architecture, documentation, and schedule for a 4-person team's CMS migration from PHP to Next.js App Router, achieving full frontend-backend separation",
      "Introduced Lighthouse CI into JSTF, a cross-team initiative setting company-wide frontend standards, so performance regressions fail CI before reaching users",
      "Mentored 2 engineers and ran an internal Next.js teaching forum, inside a 60-person Agile team where 'every line is reviewed before merge' was the operating norm",
    ],
  },
  {
    company: "AI.Book",
    ...
    highlights: [
      "Took the online photo-book editor from a blank canvas to a live product as sole developer, shipping within 6 months and iterating on drag-and-drop, resizing, and text-editing based on real usage",
      "Defined the API contract, data model, and error handling for OAuth sign-in, cart/order flow, and payment integration from the frontend consumer's perspective, later implemented by the backend",
    ],
  },
  {
    company: "FundFluent",
    ...
    highlights: [
      "Designed and published a cross-project shared UI component library (Stencil.js Web Components, framework-agnostic) to private NPM, using a monorepo that let the library be independently versioned, tested, and published apart from business code",
      "Documented the design system in Storybook so non-React consumers and designers could evaluate the component API without reading source",
    ],
  },
  {
...
const projects: Project[] = [
  {
    ...
    summary:
      "A cross-project shared UI component platform built with Web Components (Stencil.js) for framework-agnostic usage, published to private NPM and integrating TailwindCSS + Semantic UI with Storybook documentation.",
    stack: ["Stencil.js", "Web Component", "TailwindCSS", "Storybook"],
    company: "FundFluent",
    image: "https://cdn.lazy-cv.com/fundfluent-ui-kit.png",
    highlights: [
      "UI components built with Stencil as Web Components — consumable outside the React ecosystem, not tied to a single framework's type system",
      "Monorepo separating the component library from business code, so the library could be independently versioned, tested, and published",
      "Documented the public component API via Storybook so design and non-React consumers could evaluate it without reading source",
    ],
    contributions: [
      "Built and published entirely from scratch as sole developer",
    ],
  },
  {
    name: "TWSG Bilibili CDN Accelerator",
    summary:
      "An unofficial Chrome / Edge / Firefox browser extension that makes the web version of Bilibili load smoother for users in Taiwan and Singapore, dynamically rewriting the video CDN host to the fastest regional node with automatic fallback. Solely designed, built, published, and maintained end-to-end — shipped publicly and iterated on real user feedback.",
    ...
    highlights: [
      "Cross-runtime by design: a single codebase ships to Chrome, Edge, and Firefox (three distinct extension platforms) plus a trilingual UI, similar in spirit to publishing one SDK across multiple JS runtimes",
      "Automatic fallback & fault tolerance: monitors segment request status and playback stalls, silently switching to Bilibili's native backup node before ever prompting the user — a failure caught once, guarded forever",
      "Reproducible builds: pure Node.js scripts (jszip + sharp + puppeteer) package the same source into byte-for-byte identical Chrome/Edge/Firefox zips — release engineering discipline for a public artifact",
      "Open sourced on GitHub; iterated the fallback and retry logic based on real playback-failure signals from users rather than a spec",
    ],
    contributions: [
      "10 days after launch: 2,000 views, 1,000 installs (**50% conversion rate**), WAU past 1,000 (**near-100% retention**), and **14 five-star reviews with zero below five stars**, 100% organic",
  ...
  },
  {
    ...
    highlights: [
      "Neon Serverless PostgreSQL + Drizzle ORM for type-safe database access end to end",
      "AI Agent automated SOP via Chrome DevTools MCP for end-to-end resume generation, built to be legible and drivable by AI coding agents",
...
export default function SupabaseSDKEngineerResumeEN() {
  return (
    <ResumeLayout>
      ...
      <HeroProfile
        ...
        avatarFallback="JH"
        jobTitle={"Senior Software Engineer\nApply for SDK Engineer - JavaScript"}
        description={`7 years of frontend engineering experience, most recently as a software engineer at TikTok Singapore's User Growth team and previously a senior engineer at LINE Taiwan. Core stack: React, Vue, and Expert-level TypeScript, with production experience spanning a million-line monorepo and one-person library/tooling builds.

Has designed and shipped reusable, type-safe building blocks rather than one-off features: published a framework-agnostic UI component library (Stencil.js Web Components) to private NPM at FundFluent, using a monorepo that let the library be versioned, tested, and released independently of business code; at TikTok, abstracted a Web Worker validation pipeline into shared internal tooling beyond its original use case, and built cache/cacheMany memoization utilities mimicking React's useMemo for the team to reuse.

Treats testing and documentation as part of the deliverable, not an afterthought: introduced Lighthouse CI into LINE Taiwan's company-wide frontend standards so regressions fail CI before reaching users, documented the FundFluent UI Kit's public API in Storybook for non-React consumers, and authored the architecture docs and RFC-style planning for a company-wide CMS migration.

Comfortable operating autonomously and asynchronously: mentored 2 engineers and ran an internal teaching forum at LINE Taiwan, worked as a remote contractor at AI.Book and FundFluent, and communicates fluently in English, Japanese, and Mandarin — useful for an async-by-default team.

For Supabase's SDK team, this means bringing the same instinct that turned an internal utility into shared tooling and a weekend project into a publicly relied-upon extension to the JS/TS SDKs that are the front door most developers — and their AI coding agents — use to talk to Supabase.`}
        location="Singapore"
        stats={stats}
        contacts={contacts}
        globeCaption="TikTok Singapore / LINE Taiwan — Type-Safe APIs & Shared Libraries for Distributed Teams"
      />
      ...
      <ProjectsGrid
        title="Projects"
        description={`${projects.length} representative projects showcasing library authorship, type-safe API design, and publicly shipped developer tools.`}
        projects={projects}
