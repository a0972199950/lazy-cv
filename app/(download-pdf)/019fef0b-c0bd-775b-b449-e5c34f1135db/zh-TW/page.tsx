...
export const metadata: Metadata = {
  title: "John Hsieh | 應徵 Software Engineer III, React UI Developer @ JPMorgan Chase",
  description:
    "7 年前端工程經驗，具備 React/TypeScript 與資料視覺化（ECharts/D3.js）背景，並有金融科技相關產品交付經驗——應徵 JPMorgan Chase Software Engineer III, React UI Developer 職缺。",
...
const contacts: ContactItem[] = [
  ...
  { href: "https://lazy-cv.com/019fef0b-c0bd-775b-b449-e5c34f1135db/zh-TW", type: "website", label: "個人網站" },
];
...
const experiences: Experience[] = [
  {
    ...
    highlights: [
      ...
      "解決財務/報價場景的 JavaScript 浮點數精度問題，並以 ECharts 建構結算平台報表層的數據視覺化圖表",
      "將 CSV 校驗邏輯透過 Comlink RPC 轉移至 Web Worker 執行，搭配 Yup 與 Regex 驗證規則，減少 90% UI 卡頓時間",
      "打造模仿 React useMemo 的 cache/cacheMany 快取工具，在高吞吐量財務工作流程中降低 98% Total Blocking Time (TBT)",
...
const projects: Project[] = [
  {
    ...
    highlights: [
      ...
      "以 ECharts 建構結算平台報表層的數據看板與視覺化圖表",
    ],
    contributions: [
      "Web Worker 多線程 CSV 校驗：將校驗工作轉移到單獨執行緒，使用 Comlink 實現類 RPC 通訊，並以 Jest 覆蓋驗證邏輯 —— 在 10 筆資料中**減少 90% UI 卡頓時間**",
  ...
  },
  {
    ...
    highlights: [
      ...
      "公司內部早期採用 Next.js App Router 的專案之一",
    ],
    contributions: [
  ...
  },
  {
    ...
    highlights: [
      ...
      "透過 Puppeteer 瀏覽器自動化產出商店上架素材，依序切換三語系截取商店截圖",
    ],
    contributions: [
      "上架 10 天：瀏覽數 2,000、安裝數 1,000（**轉換率 50%**），WAU 突破 1,000（**留存率近 100%**），獲得 **14 筆五星好評、零負評**",
...
export default function JPMorganReactUIResumeZhTW() {
  return (
    <ResumeLayout>
      ...
      <HeroProfile
        ...
        avatarFallback="JH"
        jobTitle={"軟體工程師 - TikTok\n應徵 Software Engineer III, React UI Developer - JPMorgan Chase"}
        description={`7 年前端工程經驗，現於 TikTok 新加坡，先前 LINE Taiwan，應徵 JPMorgan React UI 工程師。核心技術棧 React + TypeScript，具資料視覺化經驗（ECharts、D3.js）——於百萬行 React 17/TypeScript monorepo 中主導財務結算平台，解決浮點數精度問題，並降低 98% TBT。金融科技背景：FundFluent 中小企貸款平台與放貸方後台，AI.Book 主導金流串接 API 設計。

LINE 60 人團隊中，帶領 4 人團隊完成 PHP 到 Next.js 架構遷移，指導 2 位新人，導入 Lighthouse CI。Lazy CV 專案獨立部署 AWS 正式環境。中英日三語流利，具跨職能溝通經驗。`}
        location="Singapore"
        stats={stats}
        contacts={contacts}
        globeCaption="TikTok Singapore / LINE Taiwan — React UI 工程與金融科技相關經驗"
      />
      ...
      <ProjectsGrid
        title="專案介紹"
        description={`${projects.length} 個代表性專案，展示 React/TypeScript UI 工程能力、資料視覺化，以及金融科技相關產品交付經驗。`}
        projects={projects}
